...
%Course Projec (Part 4: Project Task 4)
%
%
%
%This portion of the script tackles the scenario where the troposphere
%and surface get warmer and the stratosphere gets colder under global
%warming. The temperature profile of the CIRC2 and CIRC4 profiles were
%altered to reflect this scenario and the new fluxes were found keeping the
%transmittances constant.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%Change the Temperature Profiles
%Assuming that the layer pressures that are less than 100 mb represent the
%stratosphere, and pressures greater than 100 mb  represent the
%troposphere. The temperature profiles as a result will alter in the
%following way: All temperatures in the troposphere will increase by 1
%Kelvin while the stratosphere will decrease by 1 Kelvin.

CIRC2_RefTemp = data.CIRC2.ALLGAS.SFC(:,1);
CIRC4_RefTemp = data.CIRC4.ALLGAS.SFC(:,1);

%Change CIRC2 Temp Profile
for i=1:length(CIRC2_RefTemp)
    if i < 29 %cutoff for troposphere
        CIRC2_Temp(i) = CIRC2_RefTemp(i)+1;
    else
        CIRC2_Temp(i) = CIRC2_RefTemp(i)-1;
    end
end

%Change CIRC4 Temp Profile
for i=1:length(CIRC4_RefTemp)
    if i < 37 %cutoff for troposphere
        CIRC4_Temp(i) = CIRC4_RefTemp(i)+1;
    else
        CIRC4_Temp(i) = CIRC4_RefTemp(i)-1;
    end
end
...
for i = 1:length(bands)-1
    for j=1:length(data.CIRC2.CO2.SFC(:,1))-2
        Temp =  CIRC2_Temp(j+1); %temperature of each layer
        P_layer = B(v,Temp); %planck fn for each layer
        P_layer1 = B(v,CIRC2_Temp(1)); %planck fn @ layer 1
...
%Total Flux per band
newtot_CIRC2_CO2_SFC_band = sum(CO2_flux_layer);
%Total Flux 
newtot_CIRC2_CO2_SFC = sum(CO2_flux_layer, 'all');
...
for i = 1:length(bands)-1
    for j=1:length(data.CIRC4.CO2.SFC(:,1))-2
        Temp = CIRC4_Temp(j+1); %temperature of each layer
        P_layer = B(v,Temp); %planck fn for each layer
        P_layer1 = B(v,CIRC4_Temp(1)); %planck fn @ layer 1
        P_layer = matlabFunction(P_layer);
        P_layer1 = matlabFunction(P_layer1);
...
%Total Flux per band
newtot_CIRC4_CO2_SFC_band = sum(CO2_flux_layer);
%Total Flux 
newtot_CIRC4_CO2_SFC = sum(CO2_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_H2OCTM_SFC_band = sum(H2O_flux_layer);
%Total Flux 
newtot_CIRC2_H2OCTM_SFC = sum(H2O_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_H2OCTM_SFC_band = sum(H2O_flux_layer);
%Total Flux 
newtot_CIRC4_H2OCTM_SFC = sum(H2O_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_CH4_SFC_band = sum(CH4_flux_layer);
%Total Flux 
newtot_CIRC2_CH4_SFC = sum(CH4_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_CH4_SFC_band = sum(CH4_flux_layer);
%Total Flux 
newtot_CIRC4_CH4_SFC = sum(CH4_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_N2O_SFC_band = sum(N2O_flux_layer);
%Total Flux 
newtot_CIRC2_N2O_SFC = sum(N2O_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_N2O_SFC_band = sum(N2O_flux_layer);
%Total Flux 
newtot_CIRC4_N2O_SFC = sum(N2O_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_O3_SFC_band = sum(O3_flux_layer);
%Total Flux 
newtot_CIRC2_O3_SFC = sum(O3_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_O3_SFC_band = sum(O3_flux_layer);
%Total Flux 
newtot_CIRC4_O3_SFC = sum(O3_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_ALLGAS_SFC_band = sum(ALLGAS_flux_layer);
%Total Flux 
newtot_CIRC2_ALLGAS_SFC = sum(ALLGAS_flux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_ALLGAS_SFC_band = sum(ALLGAS_flux_layer);
%Total Flux 
newtot_CIRC4_ALLGAS_SFC = sum(ALLGAS_flux_layer, 'all');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%Gas TOA Flux Calculations 
...
for i = 1:length(bands)-1
    for j=length(data.CIRC2.CO2.TOA(:,1))-1:-1:2
        ...
        P_layer = B(v,Temp); %planck fn for each layer
        P_layer1 = B(v,CIRC2_Temp(1)); %planck fn @ layer 1
        P_layerN = B(v,CIRC2_Temp(end)); %planck fn @ layer N
        P_layer = matlabFunction(P_layer);
        P_layer1 = matlabFunction(P_layer1);
...
%Total Flux per band
newtot_CIRC2_CO2_TOA_band = sum(CO2_TOAflux_layer);
%Total Flux 
newtot_CIRC2_CO2_TOA = sum(CO2_TOAflux_layer, 'all');
...
for i = 1:length(bands)-1
    for j=length(data.CIRC4.CO2.TOA(:,1))-1:-1:2
        ...
        P_layer = B(v,Temp); %planck fn for each layer
        P_layer1 = B(v,CIRC4_Temp(1)); %planck fn @ layer 1
        P_layerN = B(v,CIRC4_Temp(end)); %planck fn @ layer N
        P_layer = matlabFunction(P_layer);
        P_layer1 = matlabFunction(P_layer1);
        P_layerN = matlabFunction(P_layerN);
...
%Total Flux per band
newtot_CIRC4_CO2_TOA_band = sum(CO2_TOAflux_layer);
%Total Flux 
newtot_CIRC4_CO2_TOA = sum(CO2_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_H2OCTM_TOA_band = sum(H2OCTM_TOAflux_layer);
%Total Flux 
newtot_CIRC2_H2OCTM_TOA = sum(H2OCTM_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_H2OCTM_TOA_band = sum(H2OCTM_TOAflux_layer);
%Total Flux 
newtot_CIRC4_H2OCTM_TOA = sum(H2OCTM_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_CH4_TOA_band = sum(CH4_TOAflux_layer);
%Total Flux 
newtot_CIRC2_CH4_TOA = sum(CH4_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_CH4_TOA_band = sum(CH4_TOAflux_layer);
%Total Flux 
newtot_CIRC4_CH4_TOA = sum(CH4_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_N2O_TOA_band = sum(N2O_TOAflux_layer);
%Total Flux 
newtot_CIRC2_N2O_TOA = sum(N2O_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_N2O_TOA_band = sum(N2O_TOAflux_layer);
%Total Flux 
newtot_CIRC4_N2O_TOA = sum(N2O_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_O3_TOA_band = sum(O3_TOAflux_layer);
%Total Flux 
newtot_CIRC2_O3_TOA = sum(O3_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_O3_TOA_band = sum(O3_TOAflux_layer);
%Total Flux 
newtot_CIRC4_O3_TOA = sum(O3_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC2_ALLGAS_TOA_band = sum(ALLGAS_TOAflux_layer);
%Total Flux 
newtot_CIRC2_ALLGAS_TOA = sum(ALLGAS_TOAflux_layer, 'all');
...
%Total Flux per band
newtot_CIRC4_ALLGAS_TOA_band = sum(ALLGAS_TOAflux_layer);
%Total Flux 
newtot_CIRC4_ALLGAS_TOA = sum(ALLGAS_TOAflux_layer, 'all');
...
%CIRC2 Total Changes
CIRC2_CO2_SFC_CHANGE = newtot_CIRC2_CO2_SFC - tot_CIRC2_CO2_SFC;
CIRC2_CO2_TOA_CHANGE = newtot_CIRC2_CO2_TOA - tot_CIRC2_CO2_TOA;
CIRC2_H2OCTM_SFC_CHANGE = newtot_CIRC2_H2OCTM_SFC - tot_CIRC2_H2OCTM_SFC;
CIRC2_H2OCTM_TOA_CHANGE = newtot_CIRC2_H2OCTM_TOA - tot_CIRC2_H2OCTM_TOA;
CIRC2_CH4_SFC_CHANGE = newtot_CIRC2_CH4_SFC - tot_CIRC2_CH4_SFC;
CIRC2_CH4_TOA_CHANGE = newtot_CIRC2_CH4_TOA - tot_CIRC2_CH4_TOA;
CIRC2_N2O_SFC_CHANGE = newtot_CIRC2_N2O_SFC - tot_CIRC2_N2O_SFC;
CIRC2_N2O_TOA_CHANGE = newtot_CIRC2_N2O_TOA - tot_CIRC2_N2O_TOA;
CIRC2_O3_SFC_CHANGE = newtot_CIRC2_O3_SFC - tot_CIRC2_O3_SFC;
CIRC2_O3_TOA_CHANGE = newtot_CIRC2_O3_TOA - tot_CIRC2_O3_TOA;
CIRC2_ALLGAS_SFC_CHANGE = newtot_CIRC2_ALLGAS_SFC - tot_CIRC2_ALLGAS_SFC;
CIRC2_ALLGAS_TOA_CHANGE = newtot_CIRC2_ALLGAS_TOA - tot_CIRC2_ALLGAS_TOA;

%CIRC4 Total Changes
CIRC4_CO2_SFC_CHANGE = newtot_CIRC4_CO2_SFC - tot_CIRC4_CO2_SFC;
CIRC4_CO2_TOA_CHANGE = newtot_CIRC4_CO2_TOA - tot_CIRC4_CO2_TOA;
CIRC4_H2OCTM_SFC_CHANGE = newtot_CIRC4_H2OCTM_SFC - tot_CIRC4_H2OCTM_SFC;
CIRC4_H2OCTM_TOA_CHANGE = newtot_CIRC4_H2OCTM_TOA - tot_CIRC4_H2OCTM_TOA;
CIRC4_CH4_SFC_CHANGE = newtot_CIRC4_CH4_SFC - tot_CIRC4_CH4_SFC;
CIRC4_CH4_TOA_CHANGE = newtot_CIRC4_CH4_TOA - tot_CIRC4_CH4_TOA;
CIRC4_N2O_SFC_CHANGE = newtot_CIRC4_N2O_SFC - tot_CIRC4_N2O_SFC;
CIRC4_N2O_TOA_CHANGE = newtot_CIRC4_N2O_TOA - tot_CIRC4_N2O_TOA;
CIRC4_O3_SFC_CHANGE = newtot_CIRC4_O3_SFC - tot_CIRC4_O3_SFC;
CIRC4_O3_TOA_CHANGE = newtot_CIRC4_O3_TOA - tot_CIRC4_O3_TOA;
CIRC4_ALLGAS_SFC_CHANGE = newtot_CIRC4_ALLGAS_SFC - tot_CIRC4_ALLGAS_SFC;
CIRC4_ALLGAS_TOA_CHANGE = newtot_CIRC4_ALLGAS_TOA - tot_CIRC4_ALLGAS_TOA;

%CIRC2 Band Changes
CIRC2_CO2_SFC_CHANGE_band = newtot_CIRC2_CO2_SFC_band - tot_CIRC2_CO2_SFC_band;
CIRC2_CO2_TOA_CHANGE_band = newtot_CIRC2_CO2_TOA_band - tot_CIRC2_CO2_TOA_band;
CIRC2_H2OCTM_SFC_CHANGE_band = newtot_CIRC2_H2OCTM_SFC_band - tot_CIRC2_H2OCTM_SFC_band;
CIRC2_H2OCTM_TOA_CHANGE_band = newtot_CIRC2_H2OCTM_TOA_band - tot_CIRC2_H2OCTM_TOA_band;
CIRC2_CH4_SFC_CHANGE_band = newtot_CIRC2_CH4_SFC_band - tot_CIRC2_CH4_SFC_band;
CIRC2_CH4_TOA_CHANGE_band = newtot_CIRC2_CH4_TOA_band - tot_CIRC2_CH4_TOA_band;
CIRC2_N2O_SFC_CHANGE_band = newtot_CIRC2_N2O_SFC_band - tot_CIRC2_N2O_SFC_band;
CIRC2_N2O_TOA_CHANGE_band = newtot_CIRC2_N2O_TOA_band - tot_CIRC2_N2O_TOA_band;
CIRC2_O3_SFC_CHANGE_band = newtot_CIRC2_O3_SFC_band - tot_CIRC2_O3_SFC_band;
CIRC2_O3_TOA_CHANGE_band = newtot_CIRC2_O3_TOA_band - tot_CIRC2_O3_TOA_band;
CIRC2_ALLGAS_SFC_CHANGE_band = newtot_CIRC2_ALLGAS_SFC_band - tot_CIRC2_ALLGAS_SFC_band;
CIRC2_ALLGAS_TOA_CHANGE_band = newtot_CIRC2_ALLGAS_TOA_band - tot_CIRC2_ALLGAS_TOA_band;

%CIRC4 Band Changes
CIRC4_CO2_SFC_CHANGE_band = newtot_CIRC4_CO2_SFC_band - tot_CIRC4_CO2_SFC_band;
CIRC4_CO2_TOA_CHANGE_band = newtot_CIRC4_CO2_TOA_band - tot_CIRC4_CO2_TOA_band;
CIRC4_H2OCTM_SFC_CHANGE_band = newtot_CIRC4_H2OCTM_SFC_band - tot_CIRC4_H2OCTM_SFC_band;
CIRC4_H2OCTM_TOA_CHANGE_band = newtot_CIRC4_H2OCTM_TOA_band - tot_CIRC4_H2OCTM_TOA_band;
CIRC4_CH4_SFC_CHANGE_band = newtot_CIRC4_CH4_SFC_band - tot_CIRC4_CH4_SFC_band;
CIRC4_CH4_TOA_CHANGE_band = newtot_CIRC4_CH4_TOA_band - tot_CIRC4_CH4_TOA_band;
CIRC4_N2O_SFC_CHANGE_band = newtot_CIRC4_N2O_SFC_band - tot_CIRC4_N2O_SFC_band;
CIRC4_N2O_TOA_CHANGE_band = newtot_CIRC4_N2O_TOA_band - tot_CIRC4_N2O_TOA_band;
CIRC4_O3_SFC_CHANGE_band = newtot_CIRC4_O3_SFC_band - tot_CIRC4_O3_SFC_band;
CIRC4_O3_TOA_CHANGE_band = newtot_CIRC4_O3_TOA_band - tot_CIRC4_O3_TOA_band;
CIRC4_ALLGAS_SFC_CHANGE_band = newtot_CIRC4_ALLGAS_SFC_band - tot_CIRC4_ALLGAS_SFC_band;
CIRC4_ALLGAS_TOA_CHANGE_band = newtot_CIRC4_ALLGAS_TOA_band - tot_CIRC4_ALLGAS_TOA_band;
